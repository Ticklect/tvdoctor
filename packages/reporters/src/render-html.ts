...
function renderIssue(report: TVDoctorReportV1, issue: TVDoctorIssue, expanded: boolean): string {
  ...
  const replayReady = workflow.state === "verified-replay-ready";
  const replayCommandMarkup = replayReady
    ...
    : `<p><strong>${escapeHtml(workflow.label)}.</strong> ${escapeHtml(workflow.reason ?? "A deterministic replay command is not asserted for this finding.")}</p>`;
  const reproduction = issue.reproduction.status === "available"
    ? `<p><strong>Reset:</strong> ${escapeHtml(issue.reproduction.resetStrategy)} · <strong>Confidence:</strong> ${escapeHtml(issue.reproduction.confidence)}</p><p><strong>Recorded original sequence:</strong></p><pre>${formatRemoteSequence(issue.reproduction.originalSequence)}</pre>${issue.reproduction.minimizedSequence === null ? "" : `<p><strong>Recorded minimized candidate (not executed by the replay command):</strong></p><pre>${formatRemoteSequence(issue.reproduction.minimizedSequence)}</pre>`}${replayCommandMarkup}`
    : `<p class="status unavailable">Unavailable</p><pre>${escapeHtml(issue.reproduction.reason)}</pre>${replayCommandMarkup}`;
  ...
  return `<details class="issue severity-${issue.severity}" id="${escapeHtml(issue.id)}"${expanded ? " open" : ""}>
    <summary><span class="badge severity">${issue.severity.toUpperCase()}</span><span>${escapeHtml(issue.title)}</span><span class="badge workflow">${escapeHtml(workflow.label)}</span></summary>
...
function verdictText(report: TVDoctorReportV1): string {
  ...
  const replayReady = report.issues.filter((issue) => findingWorkflow(report, issue).state === "verified-replay-ready").length;
  ...
  if (replayReady > 0) return `${String(replayReady)} ${replayReady === 1 ? "verified issue is" : "verified issues are"} replay ready`;
...
function renderNextAction(report: TVDoctorReportV1): string {
  if (report.run.status === "failed") return "Resolve the recorded run failure, then run the scan again.";
  if (report.run.status === "partial") return "Resolve the recorded blocker or exhausted budget, then run the scan again.";
  const replayReady = report.issues.find((issue) => findingWorkflow(report, issue).state === "verified-replay-ready");
  if (replayReady !== undefined) {
    return `Fix the first replay-ready finding: <a href="#${escapeHtml(replayReady.id)}">${escapeHtml(replayReady.title)}</a>.`;
  }
...
function renderReleaseChecks(report: TVDoctorReportV1): string {
  ...
  return `<details class="release-checks"><summary>Release checks</summary><div class="release-body">
    <section><h2>What this scan proved</h2><p>${automatedProof}</p><p class="meta">This statement covers only the recorded automated observations and does not certify a TV model or operating system.</p></section>
    <section><h2>Still verify on real TV hardware</h2><ul><li>D-pad reachability and visible focus on every supported device</li><li>screen reader announcements, including TalkBack or VoiceView</li><li>text scaling and readability at normal viewing distance</li><li>captions and audio-description preferences</li><li>autoplay and playback interruptions, including pause, resume, and audio focus</li><li>launch, navigation, and playback performance on representative hardware</li></ul></section>
    <section class="next-action"><h2>Next action</h2><p>${renderNextAction(report)}</p></section>
  </div></details>`;
}
...
export function renderReportHtml(report: TVDoctorReportV1): string {
  ...
  const environmentRows = Object.entries(valid.target.environment)
    ...
    .join("");
  const actionGroups = [
    ["Verified · Replay ready", "Deterministic failures with a portable replay and deterministic-failure evidence"],
    ["Verified · Replay unavailable", "Deterministic observations that need a stronger portable reproduction before automated fix validation"],
    ["Needs review", "Heuristic, inferred, or otherwise lower-confidence findings for human triage"],
    ["Setup / info", "Setup context and informational observations"],
  ] as const;
  const findingSections = actionGroups.map(([label, description]) => {
    const issues = valid.issues.filter((issue) => findingWorkflow(valid, issue).label === label);
    if (issues.length === 0) return "";
    return `<section class="issue-group"><h2>${label}</h2><p class="meta">${description}</p>${issues.map((issue, index) => renderIssue(valid, issue, label === "Verified · Replay ready" && index === 0)).join("")}</section>`;
  }).join("");
...
<body><main>
  ...
  ${findingSections || "<section><h2>No findings</h2><p>No issues were reported for the observed coverage. This is not a claim of exhaustive testing.</p></section>"}
  ${patterns.length === 0 ? "" : `<section><h2>Repeated patterns</h2><ul>${patterns.map((pattern) => `<li><strong>${escapeHtml(pattern.label)}</strong> — ${String(pattern.count)} related findings across ${String(pattern.stateCount)} screen(s); ${pattern.issueIds.map((id) => `<a href="#${escapeHtml(id)}"><code>${escapeHtml(id)}</code></a>`).join(", ")}</li>`).join("")}</ul></section>`}
  ${renderReleaseChecks(valid)}
  ...
  <p class="supporting">Developer exports: <a href="exports/portable-summary.md">portable summary</a> · <a href="exports/agent-fix-tasks.md">agent fix tasks</a>. Machine data remains in this bundle for CI and replay.</p>
</main></body></html>
`;
}
