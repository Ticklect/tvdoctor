...
export function renderReportHtml(report: TVDoctorReportV1): string {
  ...
  const environmentRows = Object.entries(valid.target.environment)
    ...
    .join("");
  const severitySections = ISSUE_SEVERITIES.map((severity) => {
    const issues = valid.issues.filter((issue) => issue.severity === severity);
    if (issues.length === 0) return "";
    const heading = severity === "critical" ? "Critical blockers" : `${severity[0]?.toUpperCase() ?? ""}${severity.slice(1)} priority`;
    return `<section class="issue-group"><h2>${heading}</h2>${issues.map((issue) => renderIssue(valid, issue)).join("")}</section>`;
  }).join("");
