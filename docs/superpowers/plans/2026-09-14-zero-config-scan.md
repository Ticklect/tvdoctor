...
- [ ] **Step 1: Add failing tests proving selection no longer clears the whole terminal and that report auto-open leaves a permanent verdict/path.** Capture writes through injected terminal/report actions rather than asserting only on source strings.
...
it("auto-opens a retained report and keeps the final path visible", async () => {
  ...
  expect(openedPath).toMatch(/report\.html$/u);
...
- [ ] **Step 3: Add a failing progress-renderer test.** A progress update containing only `{ elapsedSeconds: 18 }` must render elapsed time and must not print `Screens 0`, `States 0`, `Actions 0`, or `Findings 0`. Existing Android calls that provide all counters must retain their current full line.

- [ ] **Step 4: Run the focused CLI/product-flow tests and confirm failure.**

```text
npx vitest run packages/cli/test/cli.test.ts packages/cli/test/product-flow.test.ts
```

- [ ] **Step 5: Implement `runZeroConfigTarget()` for `web`.** It must use `deep`, all packs, default query `N`, collision-safe output, self-install Chromium when probe fails and `runtimeSetup` exists, then re-probe before scanning.
...
- [ ] **Step 6: Make progress counters optional and render only observed fields.** Android continues to call `progress.update()` with every counter. The web zero-config path sends elapsed time only until richer live audit progress is available.
...
Build the line as segments:

```ts
const segments = [`Elapsed ${elapsed}`];
if (progress.screens !== undefined) segments.push(`Screens ${progress.screens}`);
if (progress.states !== undefined) segments.push(`States ${progress.states}`);
if (progress.actions !== undefined) segments.push(`Actions ${progress.actions}`);
if (progress.findings !== undefined) segments.push(`Findings ${progress.findings}`);
```

- [ ] **Step 7: Change root dispatch in `runCli()`.** Resolve known commands first. For an otherwise unknown first token, call the target classifier. `target` routes to zero-config, `invalid` returns usage error with its message, and `not-target` keeps the existing unknown-command usage error.

- [ ] **Step 8: Simplify `runGuidedStart()`.** `start` takes zero or one target argument. Zero args prompt once with `What do you want to test? Paste a website URL or APK path:`. One arg is classified directly. Do not render Website/Android or Quick/Deep menus.

- [ ] **Step 9: Add `START_HELP_TEXT`, support both `tvdoctor start --help` and `tvdoctor help start`, and update global usage with the root target forms.**

- [ ] **Step 10: Rerun focused tests and typecheck.**
...
```text
git add packages/cli/src/zero-config.ts packages/cli/src/cli.ts packages/cli/src/start.ts packages/cli/src/interactive.ts packages/cli/src/index.ts packages/cli/test/cli.test.ts packages/cli/test/product-flow.test.ts
...
- [ ] **Step 1: Add failing scan tests for Observer onboarding.** The first observer preparation reports disabled; callback returns `wait`; test enables the observer on a later read; scan relaunches/restores the target and continues. Add timeout, abort, and `stop` cases.
...
- [ ] **Step 2: Add failing device-choice tests.** Exactly one compatible TV is auto-selected with no menu; multiple compatible TVs produce one selection; non-TV/unknown-TV/API/ABI failures are excluded with reasons; zero candidates return a managed-emulator recovery request rather than mislabelling any Android device as TV.
...
- [ ] **Step 7: If no compatible external TV exists, return a typed recovery result from the Android branch rather than failing or selecting an unsuitable device.** Task 11 will consume this result to start the managed TVDoctor device.

```ts
export type AndroidTargetResolution =
  | { readonly status: "selected"; readonly serial: string; readonly owned: false }
  | { readonly status: "managed-device-required"; readonly reasons: readonly string[] };
```

- [ ] **Step 8: Ensure `start TARGET` and root `tvdoctor TARGET` both use this same branch.** Delete obsolete platform/depth/device-confirmation wizard logic from `start.ts` once no caller uses it.
...
## Task 9: Unify reporter workflow state across HTML and coder exports
...
## Task 10: Make the HTML report findings-first and link its local exports
...
## Task 11: Add the Windows/x64 TVDoctor-managed Android TV emulator
...
- [ ] **Step 1: Write failing subprocess-boundary tests.** Cover existing AVD reuse, missing system image requiring exactly one download confirmation, declined download, SDK licence interaction through `runInteractiveTool`, Windows/x64 host gate, owned AVD naming, emulator launch args, boot-ready serial detection, cancellation cleanup, and no deletion of unrelated AVDs.
...
- [ ] **Step 4: Implement package/image discovery and explicit download confirmation.** Query installed SDK packages with `runTool`. If downloads are needed, call `confirmDownload()` once with the exact package list before invoking `runInteractiveTool(sdkmanager, packages)`. Production `runInteractiveTool` inherits stdin/stdout so Android SDK licence text and acceptance remain user-controlled; tests inject a bounded fake. Do not pipe `yes` or add auto-accept behavior.

- [ ] **Step 5: Implement owned AVD creation.** Reuse only `MANAGED_ANDROID_AVD_NAME`. If missing, invoke `avdmanager` through the interactive-tool boundary with the API 36 Android TV x86_64 image and a TV profile where supported. If `devices.xml`/profile discovery prevents creation, raise a focused emulator-setup error; do not mutate other AVDs.

- [ ] **Step 6: Implement long-lived emulator launch through `startEmulator()`.** Production launch uses a hidden process on Windows and equivalent options to the proven CI baseline:

```text
-no-snapshot-save -no-window -gpu swiftshader_indirect -noaudio -no-boot-anim -no-metrics -camera-back none -camera-front none
```

Discover the new owned serial through ADB and reuse `AndroidTvDriver.waitForDeviceReady()` for boot proof. Do not model emulator launch as a one-shot `execFile` call.
...
- [ ] **Step 8: Consume the `managed-device-required` result from Task 8.** If a TVDoctor-managed AVD/image is already ready, start it automatically; if packages are missing, ask one explicit download question. Mark the returned target `owned: true` so same-package replacement can proceed without the external-device prompt.
