...
def replace_once(path: str, old: str, new: str) -> None:
    ...
    if text.count(old) != 1:
        raise RuntimeError(f"expected exactly one literal match in {path}: {old[:80]!r}")
...
def sub_once(path: str, pattern: str, replacement: str) -> None:
    ...
    if count != 1:
        raise RuntimeError(f"expected exactly one regex match in {path}: {pattern[:100]!r}")
    target.write_text(result, encoding="utf-8")


observer = "packages/driver-android/observer/src/org/tvdoctor/observer/ObserverAccessibilityService.java"
replace_once(
    observer,
...
replace_once(observer, '                lastPackageName = boundedPackageName;\n', '')
replace_once(observer, '            lastWindowId = event.getWindowId();\n', '')
replace_once(
    observer,
...
security = "scripts/test-android-observer-security.mjs"
replace_once(
    security,
    ...
    '''  await test('an active window outside the provisioned package is reported as a redacted boundary', async () => {\n    const begin = await client.request({ type: 'begin_action', key: 'RIGHT' });\n    command(['shell', 'am', 'start', '-W', '-n', 'org.tvdoctor.observer/.SetupActivity']);\n    const response = await client.request({\n      type: 'settle_action',\n      actionId: begin.actionId,\n      timeoutMs: 2000,\n      quietWindowMs: 100,\n      noResponseGraceMs: 150,\n    });\n    assert.equal(response.state.packageName, 'org.tvdoctor.observer');\n    assert.deepEqual(response.state.nodes, []);\n    assert.equal(response.state.focused, null);\n    assert.match(response.state.windowClassName ?? '', /SetupActivity/u);\n    assert.ok(response.timing.eventsObserved > 0, 'outside-package events must be visible to settling');\n  });''',
)

print("Android V2 foreground-integrity source transformations applied.")
