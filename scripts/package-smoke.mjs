...
async function main() {
  ...
  try {
    ...
    const ciHelp = runNpx(["--no-install", "tvdoctor", "ci", "--help"], {
      cwd: consumerDirectory,
    });
    if (!ciHelp.stdout.includes("--fail-on LEVEL") || !ciHelp.stdout.includes("junit.xml")) {
    ...
    const accessibilityHelp = runNpx(["--no-install", "tvdoctor", "accessibility", "--help"], {
      cwd: consumerDirectory,
    });
    if (!accessibilityHelp.stdout.includes("streaming and")
      || !accessibilityHelp.stdout.includes("does not prove TalkBack")) {
      throw new Error("The installed CLI accessibility help is incomplete.");
    }
    ...
    process.stdout.write("installed CLI start, test, accessibility, replay, setup, ci, baseline, --help, --version, and doctor: PASS\n");
